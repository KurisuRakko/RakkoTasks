...
worker 把每轮从 IMAP 拉到的新邮件（去重之后）的**原始字节**追加写进 mbox 文件——即数据库之外
另存一份不依赖邮箱服务器的原件副本。收件箱与发件箱的邮件都进同一个文件，**只追加、永不删除**；
被 LLM 判为过滤的邮件（广告、回执等）同样保留，归档不随分类结果删改。写入只发生在 `worker`；
`web` 不挂载、不读写归档目录。逐条契约见 docs/DESIGN.md §3.1。

**目录结构**（`<根>` = 容器内 `EMAIL_ARCHIVE_DIR`，compose 固定为 `/archive`，对应
宿主机上的 `EMAIL_ARCHIVE_HOST_DIR`）：

```text
<根>/<账户邮箱>.mbox        # 该账户全部邮件的 mboxrd 文件（收件箱 + 发件箱）
<根>/<账户邮箱>.mbox.idx    # 去重索引，每行一个 Message-ID 的 sha256
```

- 账户邮箱转小写，非 `[a-z0-9@._+-]` 字符替换为 `_`。
- mbox 与 `.idx` 文件 `0600`、归档根目录 `0700`（容器内 uid 1000 写）。
- `.idx` 由 worker 自己维护，不要手工编辑或删除；删掉它会导致已归档的邮件被重复追加。

**发件箱**

worker 按 IMAP LIST 里带 `\Sent` 标记的文件夹定位发件箱，只读打开、用 `BODY.PEEK[]` 拉新邮件
追加进同一个 mbox，首次同步回补 `INITIAL_BACKFILL_DAYS` 天。发件邮件不进数据库、不跑 LLM、
不建待办；发件箱失败只告警，不影响收件箱同步与账户状态。

**归档形态与限制**

- 每封邮件前面是一行 `From MAILER-DAEMON <日期>` 分隔行；CRLF 统一成 LF。正文里以 `From `
  开头的行（以及已经带若干 `>` 的这类行）会多加一个 `>`，这是 mboxrd 转义：按 mboxrd 读取的
  客户端会去掉这一层 `>`，按旧式 mbox 读取的客户端会多显示一个 `>`——这是 mbox 格式本身的限制。
  除此之外原始字节原样保存，附件都在里面。
- 追加时对 mbox 加文件锁，worker 与回补命令可以同时运行；先把 mbox fsync 落盘再写索引，
  崩溃时最坏的结果是重复一封，绝不会漏掉一封。
- 失败处理：写盘失败只计数并告警（日志只含账户邮箱名与异常类名，不含正文），不中断同步、
  不影响入库。每轮同步摘要里有 `archive: {written, duplicates, failed}`。

**回补历史**

```bash
docker compose exec worker python -m app.cli archive-backfill [--days N] [--account ID]
```

- 默认 N = `INITIAL_BACKFILL_DAYS`。
- 收件箱、发件箱都用只读的 `BODY.PEEK[]` 拉取，不改已读状态，不碰数据库，也不动同步游标。
- 可以重复执行，已归档的只计 duplicates；邮件已经从邮箱服务器上删掉的，补不回来。
