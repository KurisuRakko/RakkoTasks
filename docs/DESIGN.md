...
- 解析：标准库 `email`，取 subject/from/to/date、text/plain 与 text/html 正文；附件只记文件名列表，
  附件内容不进数据库——开启原件归档时由 worker 另把含附件原始字节的 `.eml` 落盘（见 3.1）。

### 3.1 原件归档

- 开关：环境变量 `EMAIL_ARCHIVE_DIR`（**容器内**路径），留空即关闭；compose 给 worker 固定设为 `/archive`。
- 只有 worker 写归档；web 不挂载、不读写归档。
- 何时写：worker 每次从 IMAP 拉到**新**邮件（去重之后），把 `UID FETCH (RFC822)` 拿到的原始字节
  原样写成 `.eml`，附件原始字节天然包含在内。
- 目录结构：
  - `<根>/<账户邮箱>/<YYYY-MM-DD>/<HHMMSS>_<主题>_<12位哈希>.eml`
  - 日期与时分秒按**发信时间**（邮件 Date 头）换算到 `LOCAL_TIMEZONE` 的本地时间。
  - 缺 Date 头或无法解析的邮件落在 `<根>/<账户邮箱>/undated/<主题>_<12位哈希>.eml`。
  - 账户邮箱转小写，非 `[a-z0-9@._+-]` 字符替换为 `_`；主题只保留字母数字（含中日韩文字）、`_`、`-`，
    其余替换为 `_`，截到 50 字；12 位哈希取自 Message-ID，保证同一封重复写入时路径不变（覆盖而非重复）。
- 归档规则一句话：**待办留什么，归档就留什么**——进入待办的邮件保留原件，被过滤的邮件删除原件。
- 何时删：LLM 判为被过滤的邮件（`filtered=True`，不生成待办）并成功落库后，删除对应文件；
  删完若当天目录为空一并删除。被过滤的邮件因此会在盘上短暂存在（从拉取到分类完成，通常同一轮内）。
  过滤由 LLM 按四类判定——回执/确认、可选活动推广、广告/营销/newsletter、标题党，判定规则见 §4.1。
- 保留：进入待办的邮件保留；LLM 分类失败（`llm_state="error"`）暂按保留处理——之后重试若被过滤，届时再删。
- 权限：目录 `0700`、文件 `0600`（容器内 uid 1000 写）。
- 失败处理：写盘失败只计数并告警（日志只含账户目录名与异常类名，不含主题、正文、路径），
  不中断同步、不影响入库。每轮同步摘要里有 `archive: {written, failed, discarded}`。
- 已知限制：
...
- compose 服务：`web`（uvicorn :8000）、`worker`（`python -m app.worker`，同镜像）、
  ...
  worker 另把宿主机归档目录（`EMAIL_ARCHIVE_HOST_DIR`，默认 `./data/archive`）挂到容器内 `/archive`
  （见 3.1 原件归档），该目录须预先创建并归 uid 1000 所有（见 `deploy/README.md`）。
