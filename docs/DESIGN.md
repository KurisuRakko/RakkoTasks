...
- 列表行的标签（重要 / 分类 / 截止 / 提醒）**竖着码在行右侧的 meta 列**，不独占整行：
  ...
  竖排才不会把行撑成一段楼梯；超过两个标签折成第二列——**column 方向的 `flexWrap`
  只有容器限高时才生效**，所以配套写死 `maxHeight`（两行标签 + 行距），不写就是死配置。
  Stack 必须开 `useFlexGap`：它默认把 `spacing` 编译成相邻兄弟的 `margin-top`，换列时
  第二列的头一个标签仍是 DOM 相邻兄弟，会白白多出一截与第一列错位。
- 提醒 chip 显示最早一条 `🔔 明天 10:00`，多于一条追加 `+N`；截止 chip 原样保留，
  两者可同时出现。截止 chip 收敛成 `components/DueChip`（列表行与详情对话框**共用同一个
  组件**，配色口径只有一处，不会两边分叉），分三档：**已逾期实心主色（梅）**、**今天起
  `DUE_SOON_DAYS`=15 天内到期描边主色**、更远保持中性——两档同色系，快到期与已过期
  一眼挑得出来，又不至于让所有带截止日的条目糊成一片红。逾期不再用语义 error 色。
  标签面上只有一个孤零零的日期、逾期与否纯靠颜色，而**颜色不是可访问的信息通道**，
  所以 `aria-label` 补出「截止 X 月 Y 日，已逾期 / N 天内到期」的完整说法。
